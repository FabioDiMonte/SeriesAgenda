...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        concat: {
            ...
            init: {
                src: [
                    'target/<%= pkg.name %>-<%= pkg.version %>.js',
                    'src/init/main.js'
                ],
                dest: 'target/init-<%= pkg.name %>-<%= pkg.version %>.js'
            }
        ...
        uglify: {
            ...
            init: {
                options: { banner: '<%= opt.header %><%= opt.nl %>' },
                files: {'target/init-<%= pkg.name %>-<%= pkg.version %>.min.js': ['target/init-<%= pkg.name %>-<%= pkg.version %>.js']}
            }
    ...
    // Full tasks
    grunt.registerTask('build', ['clean:target', 'concat:packages', 'concat:main', 'concat:init', 'uglify:main', 'uglify:init', 'copy:main', 'clean:target']);
